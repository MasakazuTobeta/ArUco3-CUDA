...
int main(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        const IntOption int_options[] = {
                {"--threads", &config.num_threads_},
                {"--min-side-length-canonical", &config.min_side_length_canonical_img_px_},
                {"--adaptive-thresh-win-min", &config.adaptive_thresh_win_size_min_},
                {"--adaptive-thresh-win-max", &config.adaptive_thresh_win_size_max_},
                {"--adaptive-thresh-win-step", &config.adaptive_thresh_win_size_step_},
        };
        bool handled = false;
