...
#### WP-4.1 Step 1 の実測: S10 の要素並列化

固定費の内訳を段別に切り分けた結果 (S10 43%、Otsu 39%) を受け、まず S10 に手を付けました。

| 場面 | DGX Spark | Jetson Orin | RTX 5070 Ti |
| --- | --- | --- | --- |
| 1280x720 マーカー 4 (CPU 比) | 1.94 → **1.50** | 1.08 → **0.97** | 1.32 → **0.90** |
| 3840x2160 マーカー 4 (CPU 比) | 0.99 → **0.73** | 0.46 → **0.40** | 0.60 → **0.36** |
| 検出 0 件 (ms) | 0.572 → 0.645 | 1.053 → 1.117 | 0.211 → 0.211 |

**1280x720 マーカー 4 枚で、RTX と Jetson が GPU 優位へ転じました。** 丸めは 1 bit も変わっていません (逐語 oracle との bit 一致を維持)。

検出 0 件では共有 memory の分だけ block の起動費用が上がり、わずかに遅くなります。詳細と、最初の実装が Jetson で 7 倍悪化した経緯は [benchmark 結果まとめ](benchmark-report.md) にあります。

残りは Otsu の 2 相化 (固定費の 39%) と CUDA Graph 化 (検出 0 件の 55% を占める起動費用) です。

#### Phase 4: 最適化と評価

| ID | 内容 | 成果物 | 完了条件 | 依存 | 規模 |
| --- | --- | --- | --- | --- | --- |
| WP-4.1 | kernel 起動数と待ち時間の最適化 | `src/core`、`hybrid` | 最適化前後で正確性が不変であり、変化を測定値で示せる。転送の非同期化と S10 の要素並列化は達成済み (検出のある場面で 0.06 から 0.44 ms 短縮、丸めは不変)。Otsu の 2 相化と CUDA Graph 化は未着手 | WP-3.6 (転送の非同期化は hybrid 経路で先行実施) | L (当初 M) |
