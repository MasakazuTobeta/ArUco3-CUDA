...
#### WP-4.1 の実測: S10 と Otsu の並列化

固定費の内訳 (S10 43%、Otsu 39%) に沿って 2 段階で手を入れました。CPU 比の変化です。
...
| 検出 0 件 (ms) | 0.572 → 0.615 | 1.053 → 1.124 | 0.211 → 0.211 |

**RTX 5070 Ti は全解像度で GPU 優位、Jetson は 1280x720 マーカー 1 枚でほぼ同等 (0.99) になりました。** 丸めは 1 bit も変わっていません。

Otsu の漸化式は**並列化してはならない**部分です。guard で飛ばした反復では `mu1` が正規化されないまま持ち越されるため、経路依存量になります。逐次に残したのは 3 行だけで、倍精度の除算が 1 反復あたり 2 回から 1 回へ減りました。

検出 0 件では S10 の共有 memory の分だけ block の起動費用が上がり、わずかに遅くなります。詳細と、最初の実装が Jetson で 7 倍悪化した経緯は [benchmark 結果まとめ](benchmark-report.md) にあります。

残りは CUDA Graph 化です (検出 0 件の 55% を占める起動費用)。

#### Phase 4: 最適化と評価

| ID | 内容 | 成果物 | 完了条件 | 依存 | 規模 |
| --- | --- | --- | --- | --- | --- |
| WP-4.1 | kernel 起動数と待ち時間の最適化 | `src/core`、`hybrid` | 最適化前後で正確性が不変であり、変化を測定値で示せる。転送の非同期化、S10 の要素並列化、Otsu の 3 相化は達成済み (検出のある場面で 0.22 から 0.61 ms 短縮、丸めは不変)。CUDA Graph 化は未着手 | WP-3.6 (転送の非同期化は hybrid 経路で先行実施) | L (当初 M) |
