...
2. **CPU が上回るのは 640x480 かつ検出が 1 件以上ある場面だけです。** 28 場面のうち CPU が不利にならない場面は DGX Spark 5 件、GeForce RTX 5070 Ti 4 件、Jetson AGX Orin 1 件です (1280x720 マーカー 1 枚の同着 1 件を含む)。
3. **境界を決めるのは解像度でも候補数でもなく、二値化後の輪郭点数です。** 輪郭点 1e5 あたりの係数は CPU 2.48 から 5.35 ms、Hybrid 2.54 から 5.48 ms に対し、CUDA-Resident は 0.041 から 0.278 ms です。
4. **Hybrid の輪郭点係数は CPU とほぼ同じです。** 輪郭抽出から先を host で行うためで、ここが Hybrid と CUDA-Resident の本質的な違いです。
5. **CUDA-Resident の価値は「遅い場面が無い」ことです。** 場面による振れ幅は 3.4 から 4.1 倍で、CPU の 11.6 から 20.8 倍、Hybrid の 10.0 から 43.7 倍と比べて小さく収まります。
6. **単発の検出では CPU が桁違いに速いままです。** 1 枚目の結果が出るまで CPU は 2.2 から 6.1 ms、GPU 経路は 57.6 から 174.0 ms です。相殺には Hybrid で約 100 から 420 frame、CUDA-Resident で約 110 から 350 frame を要します。
7. **入力に managed memory を選んではいけません。** 単体 GPU で 6.4 から 30 倍遅くなります。統合 GPU では 1.01 から 1.22 倍にとどまりますが、速くはなりません。
8. **GPU 経路の実行間ばらつきは CPU 経路より 1 桁大きくなります。** 1 回だけ測った値で判断できません。
