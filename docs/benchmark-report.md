...
## WP-4.1 Step 1: S10 の要素並列化

S10 の四隅補正を「1 thread が 1 隅」から「1 block が 1 隅、要素ごとに並列」へ書き換えました。

### 端から端の p50 (ms)

| 場面 | DGX Spark | Jetson Orin | RTX 5070 Ti |
| --- | --- | --- | --- |
| 640x480 マーカー 4 | 1.234 → **1.073** | 1.684 → **1.624** | 0.698 → **0.599** |
| 1280x720 マーカー 1 | 1.339 → **0.996** | 1.692 → **1.532** | 0.779 → **0.526** |
| 1280x720 マーカー 4 | 1.351 → **1.046** | 1.820 → **1.630** | 0.808 → **0.550** |
| 1280x720 マーカー 16 | 1.491 → **1.224** | 2.176 → **1.888** | 0.938 → **0.664** |
| 1920x1080 マーカー 4 | 1.473 → **1.187** | 2.081 → **1.875** | 0.895 → **0.636** |
| 3840x2160 マーカー 4 | 1.672 → **1.232** | 2.677 → **2.336** | 1.043 → **0.620** |
| blur 1280x720 (検出 0) | 0.572 → **0.645** | 1.053 → **1.117** | 0.211 → **0.211** |
| noise 1280x720 (検出 0) | 0.700 → **0.699** | 1.250 → **1.311** | 0.224 → **0.224** |

### CPU に対する比 (1 未満なら GPU が速い)

| 場面 | DGX Spark | Jetson Orin | RTX 5070 Ti |
| --- | --- | --- | --- |
| 640x480 マーカー 4 | 2.23 → **1.94** | 1.32 → **1.28** | 1.86 → **1.59** |
| 1280x720 マーカー 1 | 2.23 → **1.66** | 1.21 → **1.10** | 1.51 → **1.02** |
| 1280x720 マーカー 4 | 1.94 → **1.50** | 1.08 → **0.97** | 1.32 → **0.90** |
| 1280x720 マーカー 16 | 1.28 → **1.05** | 0.70 → **0.61** | 0.82 → **0.58** |
| 1920x1080 マーカー 4 | 1.51 → **1.21** | 0.79 → **0.71** | 1.00 → **0.71** |
| 3840x2160 マーカー 4 | 0.99 → **0.73** | 0.46 → **0.40** | 0.60 → **0.36** |
| blur 1280x720 (検出 0) | 0.98 → **1.10** | 0.79 → **0.84** | 0.43 → **0.43** |
| noise 1280x720 (検出 0) | 0.26 → **0.26** | 0.21 → **0.22** | 0.08 → **0.08** |

**検出のある場面で 0.06 から 0.44 ms 短縮しました。** 大きい画像で効きが大きく、3840x2160 では DGX Spark で 0.44 ms、RTX 5070 Ti で 0.42 ms 縮んでいます。CPU 比では 1280x720 マーカー 4 枚が RTX で 1.32 から 0.90、Jetson で 1.08 から 0.97 へ移り、**この解像度で GPU が CPU を上回りました**。

**丸めは 1 bit も変わっていません。** 逐語 oracle との bit 一致 (退化した入力 128 隅、画像端 8 通り) と、`cv::cornerSubPix` との一致 (16/16、96/96) がすべて維持されています。
...
DGX Spark で 0.572 から 0.645 ms、Jetson で 1.053 から 1.117 ms へ増えています (RTX は変化なし)。共有 memory を 1 block あたり約 5.5 KB 使うため、**検出が 0 件でも block を起こす費用が上がった**ためです。

block 数は評価計画の上限 (マーカー 16 枚 = 64 隅) を 1 波で覆う 64 に絞りました。これを超える隅は block が跨ぎながら処理します。
