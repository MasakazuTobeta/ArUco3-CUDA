...
### subpixel 補正を切った場合の差異

`--use-aruco3 0` は ArUco3 検出戦略を無効にし、OpenCV の既定に合わせて subpixel 補正も切ります。この条件で測ると、差異の性質が変わります。
...
**subpixel 補正はこの 1 pixel の違いを吸収します。** 補正を入れると 18 件が 1 件まで減ります。残る 1 件が上記の遮蔽場面です。
...
同じ seed から生成した corpus 画像が、**aarch64 と x86_64 で一致しません**。91 場面のうち 54 場面が異なります。
...
異なる画素は場面あたり 0.1% 前後にとどまり、階調差は最大でも 4 です。corpus 生成器は OpenCV の `warpPerspective` と `GaussianBlur` を使います。SIMD 経路が architecture ごとに異なり、丸めが一致しないためと見ていますが、切り分けは済んでいません。

**この差は architecture をまたぐ数値の比較にだけ影響します。** 同じ機の中で CPU と CUDA-Resident を比べる測定には影響しません。両者は同じ画像を見ています。影響の大きさは、四隅 RMSE の 0.5184 px と 0.5042 px の差、すなわち 2.7% です。
...
- corpus 生成を architecture 非依存にするか、architecture ごとに固定した corpus を配布するかを決める。
