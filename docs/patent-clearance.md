...
| 請求項 1 の構成要件 | 本 project |
| --- | --- |
| 高次元の色空間から低次元の色空間への変換 | **無い。** 公開 API は 8 bit grayscale の `ImageViewU8` のみを受け取る。`cvtColor` / `COLOR_` / `rgb` / `yuv` / `hsv` の grep は 0 件 |
| 候補コーナ点を表すピクセルの検出 | **無い。** corner 検出器を持たない。`harris` / `goodFeatures` / `cornerHarris` の grep は 0 件。四隅は連結成分の全画素に対する距離最大化で生成する |
| 境界までのしきいピクセル距離による候補コーナ点の絞り込み | **無い。** 上記のとおり候補コーナ点そのものが存在しない |
...
7. **NVIDIA family の claim chart 作成。** `US11113819B2` / `US12322114B2` / 係属中の `US20250292411A1` の各独立 claim と、本 project の S3-S6 を要素ごとに対照することを依頼します。**第 2 次調査で claim ごとの要件の所在が公報原本により確定しています** — US12322114B2 は独立 claim 3 本すべてが「候補 corner 点と境界の閾値画素距離比較」を必須要件とし角度計算と色空間変換は従属 claim のみ、US20250292411A1 は独立 claim 3 本すべてが角度計算を必須要件とし claim 10 のみ `points` と記載されて広い。**本 repository には corner 検出器も角度計算も色空間変換も存在しない**という grep 結果 (commit `912a17c`) を入力として提示できます。`candidate corner points` という用語の解釈と、本 project の極点探索との対照が争点になります。従属 claim および継続出願の claim 変遷の監視方針も含めて依頼します。
