...
- **指定商品の類否と出所混同のおそれは判断していません。** 第 9 類に一般の computer program が無いという事実までが本調査の到達点です。類似群コードの対比と混同のおそれの評価は専門家の領域です。
- 未登録の商標としての保護 (不正競争防止法) は検討していません。
...
**著作権・license の面では公開できる状態です。商標も、名称の使用を妨げる登録は見つかりませんでした。**

残るのは特許のうち OSS 公開でも消えない部分だけです。具体的には均等論とクレーム解釈、従属請求項、間接侵害の 3 点で、いずれも [特許 Clearance 下調べ記録](patent-clearance.md) に残件として挙げてあります。
...
- `ArUco` 商標の**指定商品の類否と出所混同のおそれ**。登録の所在と指定商品は 2026-08-29 に調査済みですが (上の「商標」)、類否の判断は行っていません。
