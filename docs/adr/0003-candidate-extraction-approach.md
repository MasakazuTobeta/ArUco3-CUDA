...
[検出パイプライン設計](../design/detector-pipeline.md) が挙げた 3 案のうち、S4 と S5 (四角形候補抽出) をどれで進めるかを決めます。Phase 3 以降の実装はこの選択の上に積み上がるため、Phase 2 の完了時点で確定させます。
...
| 案 | 内容 | Phase 2 時点の状態 |
| --- | --- | --- |
| A | 連結成分ラベリングと極点探索で四隅を直接求める | 実装済み (WP-2.1 から WP-2.5) |
| B | GPU 上で境界追跡を行い、多角形近似を適用する | 未実装 |
| C | 二値化画像を host へ戻し、CPU の輪郭追跡で四隅を求める | 実装済み (WP-1.5) |
...
この差はマーカーの取りこぼしではありません。真値に対する検出はどちらの設定でも 38/38 です。案 C の余分な候補は Dictionary 照合の border 検証で落ちるものです。ただし OpenCV は代表候補の識別が失敗した場合に同じ group の別候補を試す経路 (`closeContours`) を持ち、案 A ではその代替が存在しません。影響は Phase 3 で識別まで通したうえで測ります。
...
- 案 A は輪郭の順序付けと可変長 buffer を必要とせず、label 単位で完全に並列化できる。Phase 3 の decode も label 単位であり、構造が揃う。
...
- 小さい segmentation 画像では案 C の方が速い。ArUco3 有効かつ素の場面という、本 project が主に想定する条件がそこに当たる。kernel 起動の固定費を減らす最適化が Phase 4 の課題になる。
...
次のいずれかが Phase 3 以降で確認された場合、案 B の実装または案 C への切り替えを再検討します。

- 実写 corpus で、案 A の検出率が案 C より 1% 以上低い。
- `closeContours` の欠如により、識別の成功率が案 C より有意に低い。
- Phase 4 の最適化後も、想定条件で案 C より遅い。

## 未確定事項

- kernel 起動の固定費をどこまで下げられるか。window 3 通りを 1 つの kernel へまとめる、または CUDA Graph を使う案がある。Phase 4 で測定して決める。
