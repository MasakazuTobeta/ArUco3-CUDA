...
- **Widening dictionary support.** Seventeen dictionaries are bundled, but the benchmark and accuracy figures were all taken with `DICT_ARUCO_MIP_36h12`. Cross-checks against the CPU baseline run at every marker size; the recorded performance and accuracy numbers do not. The policy is in the [Dictionary Policy](dictionaries.md).
